...
class ModelExtensionTotalCoupon extends Model {
	...
	public function getPromotion() 
	{
		...
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1' LIMIT 1");
		...
		if ($coupon_query->num_rows) {
			...
			$coupon_total = $this->getTotalCouponHistoriesByCoupon('');
			if ($coupon_query->row['uses_total'] > 0 && ($coupon_total >= $coupon_query->row['uses_total'])) {
				$status = false;
			}
			...
			if ($this->customer->getId()) {
				$customer_total = $this->getTotalCouponHistoriesByCustomerId('', $this->customer->getId());
